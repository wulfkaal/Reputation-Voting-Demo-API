...
const users = {
  ...
  update: async (req, res) => {   
    ...
    req.db.collection('users').updateOne(
      {_id: ObjectID(req.params.id)}, {$set: obj}, function (err, r) {
      if (err) {
          // If it failed, return error
  ...
  auth: async(req, res) => {
    ...
    // return (
    matchingUsers = req.db.collection("users").find(
      ...
      .toArray((err, docs) => {
        if (err || !docs.length) {
          ...
          .status(401)
          .send(
            { error: `User with publicAddress ${publicAddress} is` + 
          ...
          delete usr._id
          req.db.collection('users').updateOne(
            {_id: ObjectID(user._id)}, {$set: usr}, function (err, r) {
            if (err) {
                res
                .status(401)
